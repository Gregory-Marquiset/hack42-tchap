...
## 3bis. Tchap est fermé aux autres outils
...
par salon, et une fonction de mise en forme par source. Pour le sortant, un
Application Service reçoit déjà de Synapse tous les événements de ses salons.
Dans les deux cas, c'est un **Application Service** ce chantier repose sur le
[3](#3-pas-dapi-pour-créer-salons-et-fils).

**Ce qu'on construit.** Entrant une adresse de webhook qu'on colle dans
Alertmanager, Grafana ou GitLab, et l'alerte arrive formatée dans le salon
d'astreinte. Sortant un abonnement "tel salon, tel type de message → telle
adresse externe".

**Le piège.** Le chiffrement. Poster dans un salon public passe sans
effort; dans un salon privé, Tchap chiffre de force et l'Application Service
doit chiffrer lui-même. Synapse le permet, mais derrière deux options
expérimentales désactivées par défaut vérifié sur notre serveur. Et une
tempête d'alertes noie un salon en une minute, il faut regrouper avant de
poster.

**La marche à suivre.**
1. Réutiliser l'Application Service du chantier 3, ou déployer `matrix-hookshot` qui en est un même registre YAML, même déclaration dans `homeserver.yaml`.
2. Entrant: exposer `POST /webhook/{jeton}` derrière Caddy. Le jeton unique par salon, fait office d'authentification une adresse devinable serait un canal ouvert vers les agents publics.
3. Mettre en forme selon la source Alertmanager envoie `alerts[]` avec `status`, `labels` et `annotations` on en tire gravité résumé et lien. Puis `PUT /rooms/{id}/send/m.room.message/{txn}` au nom de l'AS.
